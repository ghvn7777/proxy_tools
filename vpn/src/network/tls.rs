...
impl TlsClientConnector {
    ...
    pub async fn connect<S>(&self, stream: S) -> Result<ClientTlsStream<S>, VpnError>
    where
        S: AsyncRead + AsyncWrite + Unpin + Send,
    {
        let dns = DNSNameRef::try_from_ascii_str(self.domain.as_str())
            .map_err(|_| VpnError::InternalError("Invalid DNS name".into()))?;
...
impl TlsServerAcceptor {
    ...
    pub fn new(cert: &str, key: &str, client_ca: Option<&str>) -> Result<Self, VpnError> {
        let certs = load_certs(cert)?;
        let key = load_key(key)?;
        ...
        let mut config = match client_ca {
            ...
            Some(cert) => {
                ...
                client_root_cert_store
                    ...
                    .map_err(|_| VpnError::CertificateParseError("CA", "cert"))?;
        ...
        config
            ...
            .map_err(|_| VpnError::CertificateParseError("server", "cert"))?;
    ...
    /// 触发 TLS 协议，把底层的 stream 转换成 TLS stream
    pub async fn accept<S>(&self, stream: S) -> Result<ServerTlsStream<S>, VpnError>
    where
        S: AsyncRead + AsyncWrite + Unpin + Send,
    {
...
fn load_certs(cert: &str) -> Result<Vec<Certificate>, VpnError> {
    let mut cert = Cursor::new(cert);
    pemfile::certs(&mut cert).map_err(|_| VpnError::CertificateParseError("server", "cert"))
}

fn load_key(key: &str) -> Result<PrivateKey, VpnError> {
    ...
    // 不支持的私钥类型
    Err(VpnError::CertificateParseError("private", "key"))
}

#[cfg(test)]
pub mod tls_utils {
    use crate::{TlsClientConnector, TlsServerAcceptor, VpnError};
    ...
    pub fn tls_connector(client_cert: bool) -> Result<TlsClientConnector, VpnError> {
        let ca = Some(CA_CERT);
    ...
    pub fn tls_acceptor(client_cert: bool) -> Result<TlsServerAcceptor, VpnError> {
        let ca = Some(CA_CERT);
        match client_cert {
