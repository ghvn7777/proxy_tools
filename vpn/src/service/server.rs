...
use super::QuicServerStreamGenerator;
...
pub async fn run_quic_server(conn: Connection, crypt: Arc<Box<dyn DataCrypt>>) -> Result<()> {
    let (main_sender, sub_senders, receivers) = channel_bus::<ServerMsg>(10, 1000);

    let (mut reader, mut writer) = QuicServerStreamGenerator::generate(conn, crypt).await?;
