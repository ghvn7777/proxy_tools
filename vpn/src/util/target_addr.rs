...
impl TargetAddr {
    pub async fn resolve_dns(self) -> Result<TargetAddr, VpnError> {
        match self {
    ...
    pub fn to_be_bytes(&self) -> anyhow::Result<Vec<u8>> {
        ...
        match self {
            TargetAddr::Ip(SocketAddr::V4(addr)) => {
                ...
                buf.extend_from_slice(&[consts::SOCKS5_ADDR_TYPE_IPV4]);
            ...
            TargetAddr::Domain(ref domain, port) => {
                ...
                if domain.len() > u8::MAX as usize {
                    return Err(Socks5Error::ExceededMaxDomainLen(domain.len()).into());
                }
    ...
    fn from(v: TargetAddr) -> Self {
        match v {
            TargetAddr::Ip(ip) => match ip {
                SocketAddr::V4(addr) => DestinationAddr {
                    addr: Some(crate::pb::destination_addr::Addr::V4(addr.to_string())),
                },
                SocketAddr::V6(addr) => DestinationAddr {
                    addr: Some(crate::pb::destination_addr::Addr::V6(addr.to_string())),
                },
            },
            TargetAddr::Domain(domain, port) => DestinationAddr {
                addr: Some(crate::pb::destination_addr::Addr::Domain(
                    crate::pb::DomainAddr {
                        domain,
                        port: port as u32,
                    },
...
impl TryFrom<DestinationAddr> for TargetAddr {
    type Error = VpnError;

    fn try_from(value: DestinationAddr) -> Result<Self, Self::Error> {
        match value.addr {
            Some(crate::pb::destination_addr::Addr::V4(v)) => {
                let addr = v
                    .parse::<SocketAddrV4>()
                    .context(AddrError::ParseIpv4Error)?;
                Ok(TargetAddr::Ip(SocketAddr::V4(addr)))
            }
            Some(crate::pb::destination_addr::Addr::V6(v)) => {
                let addr = v
                    .parse::<SocketAddrV6>()
                    .context(AddrError::ParseIpv6Error)?;
                Ok(TargetAddr::Ip(SocketAddr::V6(addr)))
            }
            Some(crate::pb::destination_addr::Addr::Domain(v)) => {
                Ok(TargetAddr::Domain(v.domain, v.port as u16))
            }
            None => Err(AddrError::Empty.into()),
        }
    }
}
...
impl TryFrom<&DestinationAddr> for TargetAddr {
    ...
    fn try_from(value: &DestinationAddr) -> Result<Self, Self::Error> {
        match &value.addr {
            ...
            Some(crate::pb::destination_addr::Addr::Domain(v)) => {
                Ok(TargetAddr::Domain(v.domain.clone(), v.port as u16))
            }
            None => Err(AddrError::Empty.into()),
        }
    }
...
pub async fn read_address<T: AsyncRead + Unpin>(
    ...
    let addr = match atyp {
        ...
        _ => return Err(AddrError::IncorrectAddressType.into()),
    };
